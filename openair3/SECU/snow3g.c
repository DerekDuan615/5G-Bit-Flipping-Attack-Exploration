...
void snow3g_ciphering(uint32_t count, int bearer, int direction, const uint8_t *key, int length, const uint8_t *in, uint8_t *out)
{
  ...
  //JOON: Print the buffer content as hexadecimal
  // LOG_W(PDCP, "%s(): Before SNOW3G (hex), Count: %u, Bearer: %d, Direction UL0 DL1: %d\n", __func__, count, bearer, direction);
  // for (int i = 0; i < length; ++i) {
  //   LOG_W(PDCP, "%02x ", (uint8_t)in[i]);
  // }
  // LOG_W(PDCP, "\n");
  ...
  // LOG_W(PDCP, "%s(): After SNOW3G (hex), Count: %u, Bearer: %d, Direction UL0 DL1: %d\n", __func__, count, bearer, direction);
  // for (int i = 0; i < length; ++i) {
  //   LOG_W(PDCP, "%02x ", (uint8_t)out[i]);
  // }
  // LOG_W(PDCP, "\n");
}
