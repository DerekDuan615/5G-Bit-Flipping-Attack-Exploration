                                    ...
                                    char *_buffer, int size)
{
  ...
  if (entity->has_ciphering) {
    ...
    if (shuffle_enable == 0){
        ...
        // Only deshuffle the DRB to gNB
        if (!( ((entity->type == NR_PDCP_DRB_AM) || (entity->type == NR_PDCP_DRB_UM)) && (entity->is_gnb == 1) )){

            // Just normal decipher
            entity->cipher(entity->security_context,
                        buffer + header_size + sdap_header_size,
                        size - (header_size + sdap_header_size),
                        entity->rb_id, rcvd_count, entity->is_gnb ? 0 : 1);        
            ...
            // print the keystream of checksum and data part
            ...
            // print the unshuffled ciphertext (checksum and payload)
            ...
            // print the unshuffled ciphertext (all)
            ...
            // print the unshuffled plaintext (all)
                                      ...
                                      int pdu_max_size)
{
  ...
  if (entity->has_ciphering) {
    ...
    if (shuffle_enable == 0){
      ...
      if ((entity->type == NR_PDCP_DRB_AM || entity->type == NR_PDCP_DRB_UM) && (entity->is_gnb == 0)) {
      ...
      if ((entity->type == NR_PDCP_DRB_AM || entity->type == NR_PDCP_DRB_UM) && (entity->is_gnb == 0)){
        ...
        // Only shuffle DRB sent by UE, for others just do simple ciphering
        if (!( ((entity->type == NR_PDCP_DRB_AM) || (entity->type == NR_PDCP_DRB_UM)) && (entity->is_gnb == 0) )) {
            // Ciphering
            entity->cipher(entity->security_context,
                        (unsigned char *)buf + header_size + sdap_header_size,
            ...
            // print the shuffled ciphertext (Only checksum and data payload) 
            ...
            // print the shuffled ciphertext (all)           
