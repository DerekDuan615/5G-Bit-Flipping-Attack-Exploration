...
/* --- 1. Helper Functions for the Keystream-based Shuffling - End --- */
...
// (2.2) Helper function which transfer bytes to binary string for checksum XOR result check
...
// (2.3) Helper function for checksum verification
...
// (2.4) Helper function for both checksum recovery (Scenario 0 in the paper)
int both_checksum_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
                          int payload_len, int checksum_cal_len, int bit1_index, int bit2_index,
                          int starting_recovery_count) {
    int recovery_count = 0;
    
    LOG_W(PDCP, "Both checksum recovery: bit1=%d, bit2=%d\n", bit1_index, bit2_index);
    
    // Validate that both bits are valid checksum bit indices (0-15)
    if (bit1_index < 0 || bit1_index > 15 || bit2_index < 0 || bit2_index > 15) {
        LOG_E(PDCP, "Invalid bit indices: bit1=%d, bit2=%d\n", bit1_index, bit2_index);
        return 0;
    }
    ...
    if (!corrected_buffer || !test_data) {
        LOG_E(PDCP, "Failed to allocate buffers in both_checksum_recovery\n");
        if (corrected_buffer) free(corrected_buffer);
        if (test_data) free(test_data);
        return 0;
    }
    
    // Only one scenario: flip both bits in the checksum field
    ...
    // Flip both bits in the checksum
    checksum_val ^= (1 << bit1_index);  // Flip first bit
    checksum_val ^= (1 << bit2_index);  // Flip second bit
    
    // Write back the corrected checksum
    corrected_buffer[pdu_start + 26] = (checksum_val >> 8) & 0xFF;
    corrected_buffer[pdu_start + 27] = checksum_val & 0xFF;
    
    LOG_W(PDCP, "Testing checksum correction: flipped bits %d and %d\n", bit1_index, bit2_index);
    
    // Verify the correction
    if (verify_correction(corrected_buffer, test_data, pdu_start, payload_start, payload_len, checksum_cal_len)) {
        // Pack recovery data - both entries point to checksum field (word index 3)
        unsigned char first_entry = ((3 & 0x3) << 4) | (bit1_index & 0xF);   // checksum word index = 3
        unsigned char second_entry = ((3 & 0x3) << 4) | (bit2_index & 0xF);  // checksum word index = 3
        ...
        // Bounds check
        if (byte_offset + 1 < size + 30) {  // Make sure we don't overflow the extended buffer
            ...
            if (byte_offset + 1 < size + 30) {  // Another bounds check
                if (bit_offset_pos <= 2) {
                ...
                recovery_count++;
                LOG_W(PDCP, "Recovery %d (both checksum): checksum_bit1=%d, checksum_bit2=%d\n",
                      recovery_count, bit1_index, bit2_index);
            } else {
                LOG_E(PDCP, "Buffer overflow prevented in second entry packing\n");
            }
        } else {
            LOG_E(PDCP, "Buffer overflow prevented in first entry packing\n");
        }
    } else {
        LOG_W(PDCP, "Checksum correction failed for bits %d and %d\n", bit1_index, bit2_index);
    }
    ...
    LOG_W(PDCP, "Both checksum recovery complete: found %d valid recoveries\n", recovery_count);
    return recovery_count;
}

// (2.5) Helper function for two block single zero pattern recovery (Scenario 4 and 11 in the paper)
int two_block_single_zero_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
    ...
    if (!corrected_buffer || !test_data) {
        LOG_E(PDCP, "Failed to allocate buffers in two_block_single_zero_recovery\n");
        if (corrected_buffer) free(corrected_buffer);
        if (test_data) free(test_data);
        return 0;
    }
    ...
    if (num_zeros == 1) {
        ...
        // (1): zero_bit as checksum error, prev_lsb as payload error (Scenario 4 in the paper)
        ...
        // (2): Both zero_bit and prev_lsb as payload errors (Scenario 11 in the paper)
        ...
        // Case 2: Two single zeros - Special cases for Scenario 4 and 11 in the paper
        // e.g. if the XOR binary pattern is '1110 1111 1011 1111', we would check each of the scenarios twice because there are two single zero bits.
        int zero1 = zero_bits[0];
        int prev1 = previous_block_lsbs[0];
        int zero2 = zero_bits[1];
        int prev2 = previous_block_lsbs[1];
        
        // (1): zero1 as checksum, prev1 as payload (Special case for Scenario 4)
        for (int payload_word = 0; payload_word < num_payload_words && recovery_count < 16; payload_word++) {
            memcpy(corrected_buffer, buffer, size);
        ...
        // (2): Both zero1 and prev1 as payload errors (Special case for Scenario 11)
        for (int pw1 = 0; pw1 < num_payload_words && recovery_count < 16; pw1++) {
            for (int pw2 = 0; pw2 < num_payload_words && recovery_count < 16; pw2++) {
                memcpy(corrected_buffer, buffer, size);
        ...
        // (3): zero2 as checksum, prev2 as payload (Special case for Scenario 4)
        for (int payload_word = 0; payload_word < num_payload_words && recovery_count < 16; payload_word++) {
            memcpy(corrected_buffer, buffer, size);
        ...
        // (4): Both zero2 and prev2 as payload errors (Special case for Scenario 11)
        for (int pw1 = 0; pw1 < num_payload_words && recovery_count < 16; pw1++) {
            for (int pw2 = 0; pw2 < num_payload_words && recovery_count < 16; pw2++) {
                memcpy(corrected_buffer, buffer, size);
...
// (2.6) Helper function for two block recovery when one block has length 1, we would conside that block correspond to checksum flip here (Scenario 1 in the paper)
int two_block_single_one_checksum_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
    ...
    // Try all 4 recovery subscenarios of Scenario 1 in the paper (one 1s block length 1, both 1s block length 1)
...
// (2.7) Helper function for two block general recovery (both LSBs as payload bits) (Scenario 8 in the paper)
int two_block_general_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
    ...
    if (!corrected_buffer || !test_data) {
        LOG_E(PDCP, "Failed to allocate buffers in two_block_general_recovery\n");
        if (corrected_buffer) free(corrected_buffer);
        if (test_data) free(test_data);
        return 0;
    }
...
// (2.8) Helper function for one block checksum recovery (handles both normal and wrap-around cases) (Scenario 2,3,6,7 in the paper)
int one_block_checksum_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
    ...
    if (!corrected_buffer || !test_data) {
        LOG_E(PDCP, "Failed to allocate buffers in one_block_checksum_recovery\n");
        if (corrected_buffer) free(corrected_buffer);
        if (test_data) free(test_data);
        return 0;
    }
    
    // (1): Block's LSB as checksum, LSB+1 as payload (Scenario 2 in the paper)
    ...
    // (2): Block's MSB as checksum, LSB as payload (Scenario 3 in the paper)
    if (recovery_count < 16) {
    ...
    // (3): MSB+1 as checksum, LSB as payload (Scenario 6 in the paper)
    if (recovery_count < 16) {
    ...
    // (4): LSB-1 as both checksum and payload bit (Scenario 7 in the paper)
    if (recovery_count < 16) {
...
// (2.9) Helper function for one block payload recovery (both flips in payload) (Scenario 9,10,13,14 in the paper)
int one_block_payload_recovery(unsigned char *buffer, int size, int pdu_start, int payload_start,
    ...
    if (!corrected_buffer || !test_data) {
        LOG_E(PDCP, "Failed to allocate buffers in one_block_payload_recovery\n");
        if (corrected_buffer) free(corrected_buffer);
        if (test_data) free(test_data);
        return 0;
    }
    
    // (1): Block's LSB and MSB+1 as payload bits (Scenario 10, 13 in the paper)
    if (recovery_count < 16) {
    ...
    // (2): LSB-1 as both payload bits (same bit in two different payload words) (Scenario 14 in the paper)
    if (recovery_count < 16) {
    ...
    // (3): Concatenated chains - LSB as one flip, find the other through intermediate XOR (Scenario 9 in the paper)
    if (recovery_count < 16) {
...
/* --- 2. Helper Functions for the Flipped Bits Identification - End --- */
                                    ...
                                    char *_buffer, int size)
{
  ...
  if (entity->has_ciphering) { 
    ...
    // Check if this entity should use keystream-based deshuffling
    int should_shuffle = ((entity->type == NR_PDCP_DRB_AM || entity->type == NR_PDCP_DRB_UM) && 
                         (entity->is_gnb == 1) && 
                         (shuffle_enable == 1));
  ...
  if ((entity->type == NR_PDCP_DRB_AM || entity->type == NR_PDCP_DRB_UM) && (entity->is_gnb == 1) && (bitflipIden_enable == 1)){
    ...
    FILE *fp = fopen("/home/derek/openairinterface5gTwoBitRecover/openair2/LAYER2/nr_pdcp/checksums.csv", "a"); // "a" for append mode
    if (fp) {
        if (min_blocks == max_blocks) {
            // Definitive count
    ...
    if (!temp) {
        ...
        // Count the number of 1s in the XOR pattern for checking whether both flips lie in the checksum field only
        int num_ones = 0;
        int bit_positions[2];  // Store positions of the two 1s

        for (int i = 0; i < 16; i++) {
            if ((xor_cksum >> i) & 1) {
                if (num_ones < 2) {
                    bit_positions[num_ones] = i;
                }
                num_ones++;
            }
        }

        // Checking whether both flips lie in the checksum field only
        if (num_ones == 2) {
            LOG_W(PDCP, "Exactly two 1s detected at positions %d and %d - attempting both checksum recovery\n", 
                bit_positions[0], bit_positions[1]);
            
            int both_checksum_recoveries = both_checksum_recovery(buffer, size, pdu_start, payload_start,
                                                                payload_len, checksum_cal_len, 
                                                                bit_positions[0], bit_positions[1],
                                                                recovery_count);
            recovery_count += both_checksum_recoveries;
            LOG_W(PDCP, "Both checksum recovery found %d recoveries, total so far: %d\n", 
                both_checksum_recoveries, recovery_count);
            
            if (recovery_count == 0) {
                // No recoveries found - store a specific pattern for two-bit checksum case
                unsigned char pattern_entry = ((3 & 0x3) << 4) | (0 & 0xF);  // [3,0] pattern
                ...
                LOG_W(PDCP, "No recoveries found in both checksum case - stored pattern [3,0]\n");
            }
        }
                                      ...
                                      int pdu_max_size)
{
  ...
  if (entity->has_ciphering) {   
    ...
    // Check if this entity should use keystream-based shuffling
    int should_shuffle = ((entity->type == NR_PDCP_DRB_AM || entity->type == NR_PDCP_DRB_UM) && 
                         (entity->is_gnb == 0) && 
                         (shuffle_enable == 1));
